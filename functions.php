...
require_once GELIKON_DIR . '/inc/recaptcha.php';
	...
	<style id="gelikon-catalog-dropdown-styles">
		...
		cursor: pointer;
			...
			.gl-catalog-dropdown__grid {
				position: relative;
				display: block;
				height: var(--gl-catalog-mobile-height, min(520px, calc(100dvh - 16px)));
				min-height: 0;
				overflow: hidden;
				background: #fff;
			}
...
add_action('wp_enqueue_scripts', function () {
	...
	$catalog_stylesheet = get_template_directory() . '/assets/css/catalog.css';

	wp_enqueue_style(
		'gelikon-catalog',
		get_template_directory_uri() . '/assets/css/catalog.css',
		['gelikon-main'],
		file_exists($catalog_stylesheet)
			? filemtime($catalog_stylesheet)
			: wp_get_theme()->get('Version')
	);
