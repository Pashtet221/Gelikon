...
function gelikon_category_path($value) {
    ...
    $items = array_filter($items, function($item) {
        return mb_strtolower($item) !== 'главная';
    });
...
/**
 * Recalculate print product price through the n8n workflow used by
 * Print Product Workflow for WooCommerce options.
 */
add_action('wp_ajax_gelikon_recalculate_print_product_price', 'gelikon_recalculate_print_product_price_ajax');
add_action('wp_ajax_nopriv_gelikon_recalculate_print_product_price', 'gelikon_recalculate_print_product_price_ajax');

function gelikon_recalculate_print_product_price_ajax() {
	check_ajax_referer('gelikon_print_product_price', 'nonce');

	$product_id = isset($_POST['product_id']) ? absint(wp_unslash($_POST['product_id'])) : 0;
	$options_raw = isset($_POST['options']) ? wp_unslash($_POST['options']) : '{}';
	$options = json_decode($options_raw, true);

	if (!$product_id || !is_array($options)) {
		wp_send_json_error([
			'message' => __('Некорректные параметры товара.', 'gelikon'),
		], 400);
	}

	if (!function_exists('wc_get_product')) {
		wp_send_json_error([
			'message' => __('WooCommerce недоступен.', 'gelikon'),
		], 503);
	}
	...
	if (!$product) {
		wp_send_json_error([
			'message' => __('Товар не найден.', 'gelikon'),
		], 404);
	}

	$response = wp_remote_post('https://n8n.denkolapi.ru/webhook/817978ff-18b5-45d2-a6ba-a1a436e39c6e', [
		'timeout' => 12,
		'headers' => [
	...
	foreach (['price', 'total_price', 'final_price', 'amount'] as $price_key) {
		if (isset($data[$price_key]) && is_numeric($data[$price_key])) {
			$price = (float) $data[$price_key];
			break;
		}
	}

	$price_html = !empty($data['price_html']) ? wp_kses_post($data['price_html']) : '';

	if (!$price_html && null !== $price) {
