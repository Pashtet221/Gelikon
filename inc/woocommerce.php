...
function gelikon_catalog_sorting_options() {
	return [
		...
		'menu_order' => __('Порядок WooCommerce', 'gelikon'),
...
function gelikon_get_manual_catalog_product_ids() {
	$raw_ids = (string) get_theme_mod('gelikon_catalog_manual_product_ids', '');
...
function gelikon_apply_manual_catalog_product_order($orderby, $query) {
	$manual_ids = $query->get('gelikon_manual_product_ids');
	...
	$manual_ids = array_values(array_unique(array_filter(array_map('absint', $manual_ids))));
	...
	foreach ($manual_ids as $position => $product_id) {
		$cases[] = sprintf('WHEN %d THEN %d', $product_id, $position);
	}

	$manual_order_sql = sprintf(
		'CASE %1$s.ID %2$s ELSE %3$d END ASC',
		$wpdb->posts,
		implode(' ', $cases),
		count($manual_ids)
	);

	return $manual_order_sql . ', ' . $wpdb->posts . '.menu_order ASC, ' . $wpdb->posts . '.post_title ASC';
}
add_filter('posts_orderby', 'gelikon_apply_manual_catalog_product_order', 10, 2);
