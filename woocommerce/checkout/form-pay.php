...
<form id="order_review" method="post" class="gl-order-pay woocommerce-checkout" action="<?php echo esc_url($order->get_checkout_payment_url()); ?>">
	...
	<div class="gl-order-pay__shell">
		<section class="gl-order-pay__card">
			<p class="gl-order-pay__desc gl-order-pay__desc--lead"><?php esc_html_e('Проверьте состав заказа и выберите удобный способ оплаты.', 'gelikon'); ?></p>
...
<script>
jQuery(function($) {
	const $payment = $('.gl-order-pay__payment #payment');

	function refreshPaymentBoxes($changedInput) {
		const $methods = $payment.find('ul.payment_methods li');

		$methods.each(function() {
			const $method = $(this);
			const $input = $method.find('input.input-radio').first();
			const $box = $method.find('div.payment_box').first();
			...
			if ($input.is(':checked')) {
				if ($changedInput && $input.is($changedInput)) {
	...
	$payment.on('change', 'input.input-radio[name="payment_method"]', function() {
		refreshPaymentBoxes($(this));
	});
});
</script>
